...
def main():
    # Local (cluster) folders - use forward slashes on Linux
    folder2 = '/scratch/uceerjp/gwdataset/eegGH'
    folder1 = '/scratch/uceerjp/gwdataset/eegLS'
    ...
    config = {
        ...
        'locsdir': '/scratch/uceerjp/gwdataset',   
        'classselect': 'Rest',
        'plots': 1,
        ...
        'features': {
            ...
            'icoh_method': 'fft',    # 'fft' (fast) or 'welch' (slower)
            'icoh_freq_decimation': 1,  # corrected spelling
    ...
    if config['alias'] == 'LS':
        config['bpf'] = [55, 85]
        config['dir'] = '/scratch/uceerjp/gwdataset/eegLS'
        config['patient'] = LS
    elif config['alias'] == 'GH':
        config['bpf'] = [1, 5]
        config['dir'] = '/scratch/uceerjp/gwdataset/eegGH'
        config['patient'] = GH

    # Debug prints to help if something is missing
    print("CONFIG 'dir':", config['dir'])
    print("CONFIG 'locsdir':", config['locsdir'])
    print("Number of patient files (sample):", len(config['patient']))
